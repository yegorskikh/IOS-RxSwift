...
fileprivate func cachedFileURL(_ fileName: String) -> URL {
...
class ActivityController: UITableViewController {
  ...
  private func fetchEvents(repo: String) {
    ...
    response
      ...
      .flatMap { response, _ -> Observable<String> in
        guard
          let value = response.allHeaderFields["Last-Modified"] as? String
        else {
  ...
  private func processEvents(_ newEvents: [Event]) {
    ...
    if let eventsData = try? JSONEncoder().encode(updatedEvents) {
